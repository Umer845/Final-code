...
st.title("🚗 Motor Insurance Premium Prediction - Training Using CatBoost Regressor")
...
if uploaded_file is not None:
    ...
    if st.button("🚀 Train Model", key="train_btn"):
        ...
        prompt = f"""
        You are a data science expert. The CatBoost regression model trained for motor insurance premium prediction produced:
        - R²: {r2:.4f}
        - MAE: {mae:,.2f}
        - RMSE: {rmse:,.2f}
        ...
        joblib.dump(model, "models/CatBoost_premium_model_LLM.pkl")
        joblib.dump(feature_cols, "models/model_features_CatBoost_LLM.pkl")
        joblib.dump(categorical_cols, "models/model_cat_features_CatBoost_LLM.pkl")
        ...
        try:
            ...
            if engine is not None:
                results_df = pd.DataFrame([{
                    "model_name": "CatBoostRegressor + Local LLM",
                    "r2_score": round(r2, 4),
                    "mae": round(mae, 4),
                    "rmse": round(rmse, 4),
