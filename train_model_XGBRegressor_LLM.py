...
st.title("🚗 Motor Insurance Premium Prediction - Training Using XGBoost + Local LLM")
...
if uploaded_file is not None:
    ...
    if st.button("🚀 Train Model", key="train_btn"):
        with st.spinner("Training XGBoost model... Please wait ⏳"):
            model = XGBRegressor(
                n_estimators=500,
                learning_rate=0.07,
                max_depth=10,
        ...
        prompt = f"""
        You are a data science expert. The XGBoost regression model trained for motor insurance premium prediction produced:
        - R²: {r2:.4f}
        - MAE: {mae:,.2f}
        - RMSE: {rmse:,.2f}
        ...
        joblib.dump(model, "models/xgboost_premium_model_LLM.pkl")
        joblib.dump(feature_cols, "models/model_features_xgb_LLM.pkl")
        joblib.dump(categorical_cols, "models/model_cat_features_xgb_LLM.pkl")
        ...
        try:
            ...
            if engine is not None:
                results_df = pd.DataFrame([{
                    "model_name": "XGBRegressor + Local LLM",
                    "r2_score": round(r2, 4),
                    "mae": round(mae, 4),
                    "rmse": round(rmse, 4),
