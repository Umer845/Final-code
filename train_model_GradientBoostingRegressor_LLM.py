...
from sklearn.ensemble import GradientBoostingRegressor   # ✅ Use Gradient Boosting instead of XGBoost
import joblib
import os
...
st.title("🚗 Motor Insurance Premium Prediction -Training Using GradientBoost Regressor")
...
if uploaded_file is not None:
    ...
    if st.button("🚀 Train Model", key="train_btn"):
        with st.spinner("Training Gradient Boosting model... Please wait ⏳"):
        ...
        st.subheader("📊 Model Performance GBM")
        ...
        prompt = f"""
        You are a data science expert. The GBoost regression model trained for motor insurance premium prediction produced:
        - R²: {r2:.4f}
        - MAE: {mae:,.2f}
        - RMSE: {rmse:,.2f}
        ...
        joblib.dump(model, "models/GBoost_premium_model_LLM.pkl")
        joblib.dump(feature_cols, "models/model_features_gb_LLM.pkl")
        joblib.dump(categorical_cols, "models/model_cat_features_gb_LLM.pkl")
        ...
        try:
            ...
            if engine is not None:
                results_df = pd.DataFrame([{
                    "model_name": "GBRegressor + Local LLM",
                    "r2_score": round(r2, 4),
                    "mae": round(mae, 4),
                    "rmse": round(rmse, 4),
